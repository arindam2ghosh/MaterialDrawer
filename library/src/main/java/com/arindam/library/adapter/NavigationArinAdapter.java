...
public class NavigationArinAdapter extends BaseAdapter {
	...
	public NavigationArinAdapter(Context context, List<NavigationArinItemAdapter> list, List<Boolean> extraRemove, List<Integer> extra) {
		this.mList = list;		
	...
	@Override
	public NavigationArinItemAdapter getItem(int position) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        NavigationArinItemAdapter item = mList.get(position);
		...
		if (holder.icon != null) {
			if (item.icon != 0) {
                ...
                holder.icon.setVisibility(View.VISIBLE);
                holder.icon.setImageDrawable(item.dIcon);
                setAlpha(holder.icon, (!item.isHeader && item.checked ? 1f : 0.54f));
